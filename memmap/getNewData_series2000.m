...
if eegSession.btDataStreamReady==1 %don't start recording until we're ready (e.g. the serial port has had a moment)
    ...
    if (size(eegSession.D,2)>=EEG_Config.frameSize*2) %if we've accumulated 2 full frames (or more) then take the first frame off the front of the vector
     ...
     %*********Start Data To Write****************
    if(eegSession.dataFrameIndex>=whereAt) 
    currentEEGFrame(1:EEG_Config.numChans,:)=eegD.data(:,whereAt-(frameSize-1):whereAt);
    currentEEGFrame(EEG_Config.numChans+1,:)=eegD.time(whereAt-(frameSize-1):whereAt);
    whereAt=whereAt+frameSize;
    %currentEEGFrame                                
    memFile.Data.d(:,:)=currentEEGFrame;
    %memFile.Data.d
    end
