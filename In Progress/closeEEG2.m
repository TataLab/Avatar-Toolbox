...
eegD.originalTimes = eegD.time2;

time_doubles2 = typecast(eegD.time2,'double'); %interp1 only works on doubles, grr

vectorOfTimeStamps2 = find(time_doubles2 ~=0);  %find the indices where we do have time stamps (should start at 1 and go every 16th)


for i = 2 : length(vectorOfTimeStamps2)

   x = vectorOfTimeStamps2(i-1:i); %two elements at either end of the region we have to fill in
   
   yi = interp1(x,time_doubles2(x),x(1):x(2));  %linear interpolation between either end of the region
   
   yi_uint64 = typecast(yi,'uint64'); %cast back to uint64, note you can't use double() and uint64() to do this because it loses precision (for some reason)
   
   eegD.time2(x(1):x(2))=yi_uint64; %replace with the linear interpolation and typecast back to uint64
